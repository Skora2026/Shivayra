...
<div class="container-fluid p-0">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h2 class="fw-700 text-dark mb-1">{{ __('labels.create_new_user') }}</h2>
            ...
            <form action="{{ route('admin.users.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="first_name" id="first_name" class="form-control rounded-3" value="{{ old('first_name') }}" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="last_name" class="form-label fw-600 text-dark">{{ __('labels.last_name') }}</label>
                        <input type="text" name="last_name" id="last_name" class="form-control rounded-3" value="{{ old('last_name') }}">
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control rounded-3 bg-light" value="{{ __('labels.user') }}" readonly>
                        <small class="form-text text-muted">Accounts created here are always customers — there is a single store owner.</small>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="status" id="status" class="form-select rounded-3" required>
                            <option value="active" {{ old('status') === 'active' ? 'selected' : '' }}>{{ __('labels.active') }}</option>
                            <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>{{ __('labels.inactive') }}</option>
                        </select>
                    </div>
                </div>
